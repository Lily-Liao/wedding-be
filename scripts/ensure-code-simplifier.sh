#!/usr/bin/env bash
# ensure-code-simplifier.sh — 檢查 code-simplifier plugin 安裝狀態
...
if [ -n "${CACHE_MATCH}" ]; then
    ...
    if [ -n "${VERSION_DIR}" ] && [ -d "${VERSION_DIR}" ]; then
        ...
        echo ""
        echo "[${PLUGIN_NAME}] Code Simplifier is installed."
        exit 0
    fi
fi
...
# --- 未安裝 ---
echo "[${PLUGIN_NAME}] Code Simplifier is NOT installed."
echo ""
echo "Code Simplifier 是 Claude Code plugin，需透過 Claude Code 內建指令安裝："
echo ""
